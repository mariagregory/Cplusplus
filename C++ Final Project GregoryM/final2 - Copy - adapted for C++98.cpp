...
int main() {
	...
	float **arr2Dptr; // declare a pointer to 2D-array and initialize it with nullptr
	...
	if(!file) { // if the file could not be opened
		...
		delete arr2Dptr; // free the memory taken by dynamic pointers before exitting the program
...
void setSize(int &rows, int &cols) {
	...
	do { // the 1st loop is for taking and validating user's input for "rows" 
		...
		for(short next=0; next<input.length(); next++) { // step through all characters of user's input
			sym = input[next];
	...
	do { // the 2nd do-while loop is identical, but it's for taking and validating user's input for "cols" 
		...
		cin >> input; // store user's input in input string variable
		char sym;
		for(int next=0;next<input.length();next++) { // step through all characters of user's input
			sym = input[next];
