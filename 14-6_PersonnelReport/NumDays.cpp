...
/// a constructor simply calls stHours function, which sets hours and updates days
NumDays::NumDays(short hours) { stHours(hours); } 

void NumDays::stHours(short hours) { /// sets # of hours (worked or as a leave)
    this->hours = hours>=0 ? hours : (-1)*hours;
    days = static_cast<float>(this->hours)/8.0;
}
void NumDays::stDays(short days) { /// sets # of days (worked or as a leave)
    this->days = days>=0 ? days : (-1)*days;
    this->hours = this->days * 8;
}
/// add hours of 2 workers
NumDays NumDays::operator+(NumDays &other) {
    if(other.hours>0) { NumDays third(hours+other.hours); return third; }
    else return *this;
}
/// subtract the 2nd worker's hours from the 1st one
NumDays NumDays::operator-(NumDays &other) {
