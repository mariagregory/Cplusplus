/* 
 * File:  9.6 United Cause, modification 1
 Author: Mariia Gregory
 *
 * Created on March 7, 2018, 9:12 PM
 * Purpose: Store user-specifined amounts of donations in an array in the order they were received
            ...
            Sort the values in the 2nd array in ascending order.
...
// Function prototypes
 ...
 bool sorted(int *, short); // check if the array is already sorted in ascending order
 void selSort(int *, short); // sorts values in ascending order
...
bool sorted(int *array, short size) {
    for(short i=0; i<(size-1); i++) {
        // Check if the next scanned value is less than the previous
